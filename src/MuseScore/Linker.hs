...
-- The following are all slightly broken - an experiment with using LaTeX (as with the previous),
...
maketex env bk = (cct [header, tunes, raw_tex_footer]) <$ bk where
    ...
    tune (name, path) =
        TE.replace "CHART_PDF_MARKER_STRING" (TE.pack $ toFilePath $ path) $
...
pdftex env bk = do
        ...
        pure (bookpdf <$ bk)
    where
        pdfpath = let u = (tex_d env) </> (filename $ dat bk) in
            case replaceExtension ".pdf" u of
                Just path -> path
                _         -> throw (IllegalPDFFile (TE.pack $ show u))
...
\geometry{top=0mm, bottom=20mm, headheight=5mm, footskip=114pt} % don't set this manually else geometry won't know!

\begin{document}

% Use the fancy Stompin' At Summerhall logo for the title of the books
\title{Stompin at Summerhall}
